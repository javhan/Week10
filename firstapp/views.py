...
class SecondView(View):
    def get(self, request, param): #the param word is dependent on what is written in the urls
        # Grab query from url query
        query = request.GET.get("query", "no query") ## Grabs query from url
        month = request.GET.get("month", "no month") 
        return JsonResponse({"param": param, "query": query, "month" : month})
